	...
	def Move(self, direction, delay, steps = 0):
		...
		if steps > 0:
			for i in range(steps):
				...
				if direction and self.getDistance() >= upperLimit and upperLimit:
					#Create an out of bounds exception
					raise Exception
				#If moving in the negative direction, check if we are past the lower limit
				elif not direction and self.getDistance() <= lowerLimit and lowerLimit:
					#Create an out of bounds exception
					raise Exception
				#If not past our limits, move to the appropriate steps
				else:
					self.moveStep()
		elif not steps and (upperLimit and direction) or (lowerLimit and not direction):
			while not(direction and self.getDistance() >= upperLimit and upperLimit):
				self.moveStep()
			while not(not direction and self.getDistance() <= lowerLimit and lowerLimit):
				self.moveStep()
